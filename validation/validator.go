...
func init() {
	RegisterAllCustomValidators(ValidatorUtils)
}
...
func Validate(validate *validator.Validate, data any) map[string]string {
	...
	if err != nil {
		for _, v := range err.(validator.ValidationErrors) {
			switch v.ActualTag() {
			case ValidatorUtils.CheckValidator[v.ActualTag()]:
				messageMethod := ValidatorUtils.ValidatorMessage[v.ActualTag()]
				res[v.StructField()] = messageMethod(v)
			default:
